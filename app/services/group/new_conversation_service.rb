class Group::NewConversationService
  ...
  def call
    ...
    if new_group_conversation.save
      ...
      pchat_oppsed_user.new_group_conversations << new_group_conversation
      new_user_to_chat_conversations << new_group_conversation
  ...
  def create_initial_message(creator, arr_of_users_ids, new_group_conversation)
    message = Group::Message.create(
      user_id: creator.id,
      content: 'Conversation created by ' + creatror.name,
